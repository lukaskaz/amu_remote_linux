...
void Handle_USBAsynchXfer (void)
{
  ...
  if(USB_Tx_State != 1)
  {
    ...
    if(USB_Tx_ptr_out == USB_TX_DATA_SIZE) {
      USB_Tx_ptr_out = 0;
      USB_Tx_length = USB_Tx_ptr_in;
    }
    else if(USB_Tx_ptr_out > USB_Tx_ptr_in) { 
    ...
    if (USB_Tx_length > VIRTUAL_COM_PORT_DATA_SIZE)
    {
      ...
      USB_TrTx_length = VIRTUAL_COM_PORT_DATA_SIZE;
    ...
    UserToPMABufferCopy(&USB_Tx_Buffer[USB_Tx_ptr], ENDP1_TXADDR, USB_TrTx_length);
    SetEPTxCount(ENDP1, USB_TrTx_length);
    SetEPTxValid(ENDP1); 
  }  
